...
## V2.2 P2B2 当前缺口与边界（2026-09-15）

P2B2 的 objective profile 已有机器合同，但没有把它误报为 placement engine。
当前只可使用 5 条 SHOULD 的计数和有条件 loading-side metadata；material flow、
circulation、人员/车辆分离距离仍等待真实 route，compactness、shape regularity、
unused-site efficiency 保持关闭。

```ini
OBJECTIVE_PROFILE_IMPLEMENTED=true
OBJECTIVE_AGGREGATION=LEXICOGRAPHIC
WEIGHTED_SCORE=false
ROUTE_PROXY_ALLOWED=false
...
P2C 仍需单独实现候选生成、硬约束优先筛选、真实路线依赖的目标评估和最终确定性
选择；本轮不关闭这些工程实现缺口。
...
P1C_STATUS=MERGED
P1D1_STATUS=OWNER_AUTHORITY_DRAFT_REVIEW
...
当前工作为独立授权的 V2.2 P1D1 Owner authority，参见
...
上述为P1C历史阶段。P1D1仅增加出货定尺、办公室两条邻接；记录包材部分authority，不猜行列。
当前8区定尺/4区BLOCKED，旧七区几何不变；覆膜、包材、更衣、办公室仍缺定尺authority。
当前 evidence 见 `docs/tasks/V2_2-P1D1-remaining-zone-owner-authority.md`。
...
No runtime P0 issue is opened by this lane. The V1.9 P0 contract records the
frozen nine-zone minimum-estimation basis and reuses existing zone-plan
lineage. Its historical authorization snapshot remains preserved, while the
separately authorized P1 implementation is now merged and complete. The
earlier wrong-remote configuration was corrected before baseline push and is
not a remaining repository state issue.
