...
orchestration path that must exist before Task 11 Phase B's
`baseline-feasible` scenario can produce a real `success` outcome.

It is intentionally separate from PR #21 (Task 11 evaluation
pilot readiness, Draft / Open / Not merged) and from Issue #22
(transport-B E2E persistence, CLOSED via PR #33). The capability
delivered here is the standalone application orchestration that
PR #21's evaluation harness must consume as a real production
producer — not as an evaluation-owned seam.
...
PR #21 / Task 11 Phase B is currently blocked by the harness-level
blocker `EVAL_PRODUCTION_PIPELINE_PREREQUISITE_MISSING`, which
asserts a missing capability:
...
real production path not yet exposing a public entry point that:

1. Accepts an **approved ProjectVersion** as the single source of
   truth for inputs and coefficients.
...
3. Persists a **SourceBindingRecord** whose combined source hash
   is reproducible against `SourceBindingVerifier`.
4. Emits results in a shape that **SchemeService** can consume
   unchanged to produce a `SchemeRun` — i.e. without any
   hand-written snapshot, raw ORM insert, or
   "latest-row-fallback" lookup.
...
8. **Modifying `evaluation/manifest.json` or
   `evaluation/expected/*.v1.json`** to accept `review_required`
   as a baseline success substitute.
...
│   └── ...
├── domain/
│   ├── contracts.py                              ← existing (OrchestrationRequestCommand, etc.)
│   ├── snapshots.py                              ← existing (SourceSnapshotContentV1)
│   ├── fingerprint.py                            ← existing (result_hash)
│   └── errors.py                                 ← existing (OrchestrationDomainError, etc.)
├── infrastructure/
...
begins; their signatures are stated here as a contract, but the
implementation is **NOT** part of this design task:
...
- `ProductionCalculatorPort` — adapters must implement
  `execute(calculation_type, attempt_id, inputs, snapshot)` for
  each of `zone | cooling_load | equipment | power | investment`.
  Each adapter wraps the existing calculator
...
- `ApprovedNonDemoCoefficientResolverPort` — selects approved
  ...
  execution snapshot. Returns `(CoefficientContext, warnings)`.
  Demo coefficients (`source_type=demo`) are NOT acceptable for
  any baseline / production input set; the port rejects them
  with `CoefficientNotApprovedError`.
- `SchemeServiceReadyOutputPort.write(
  attempt_id, binding_id, snapshots) → SchemeRunId`
  — single integration point with `SchemeService`. Writes a
  pre-`SchemeRun` projection containing the five
  `ResultSnapshotV1`s and the verified binding; does NOT itself
  invoke the full `SchemeService.run()` (that remains a separate
  future task), but emits a deterministic representation that
  `SchemeService` can consume unchanged once Task 11 Phase C is
  authorized.

### 4.3 Adapters (high-level shape, not implementation)

- `SqlApprovedProjectVersionReadAdapter`
  — reads from the existing project module via its read port
  (no direct ORM dependence in the orchestration module).
- `SqlOrchestrationIdempotencyAdapter`
  — uses an existing schema table or extends
  `orchestration_attempts` to add `idempotency_key` (migration
  out of scope here; design only).
- `ProductionCalculatorAdapters` (one per type)
  — wraps the existing deterministic Python calculators; each
  adapter is a `CalculatorPort` implementation and inherits the
  Transaction B UoW contract.
- `CatalogApprovedCoefficientResolverAdapter`
  — uses the catalog module's read port to resolve coefficients;
  rejects `source_type=demo`, `validity_status=unverified`, or
  `requires_review=true at the coefficient level` for any
  baseline / production path.
- `SqlSchemeServiceReadyOutputAdapter`
  — persists a typed `SchemeServiceReadyOutputV1` row with the
  five snapshot payloads and the verified binding id.
...
- `database_backend: Literal["sqlite", "postgresql"]` —
  recorded in `OrchestrationRunAttempt.database_backend`.
- `actor_principal_type: Literal["user", "service"]`.

These are persisted on `OrchestrationIdentity` /
`OrchestrationRunAttempt` and propagated to every outbox event
envelope. They MUST NOT be reconstructed or guessed on retry.

### 4.5 Run status lifecycle

```
PENDING → RUNNING → (
  SUCCEEDED                       (binding verified, archive written)
  | BLOCKED                       (preflight or coefficient rejection)
  | FAILED                        (calculator failure, no partial state)
  | IDEMPOTENT_REPLAY_SUCCEEDED   (replay of an existing SUCCEEDED attempt)
)
```

Transitions are terminal. Replay of a SUCCEEDED attempt by
idempotency key MUST NOT re-execute calculators or re-persist
records — it MUST return the existing result, marked
...
- `orchestration_requests`, `orchestration_attempts`,
  ...
  `execution_snapshots`, `calculation_runs`, `source_bindings`,
  `source_archive`, `audit_outbox`, `weight_set_revisions`,
  `audit_events`.
...
- New `scheme_service_ready_outputs` table holding
  `(attempt_id PK FK, binding_id, snapshot_zone, ...,
  snapshot_investment, scheme_ready_at, frozen_envelope)`.
  `frozen_envelope` is the canonical
  `SourceSnapshotContentV1` JSON, frozen at write time.

All schema changes MUST be Alembic-versioned and accompanied by
a downgrade/re-upgrade roundtrip test on PostgreSQL.
...
- Emit `Review-Required` only when a real warning comes from
  a calculator adapter (e.g. equipment-margin-warning) — NEVER
  as a default.
...
- Decimal canonicalisation: `Decimal` values are serialised as
  `str` (canonical string) and re-parsed losslessly. `float`
...
- Hash coverage: `result_hash = sha256(canonical_json(
  ...
  decimal=str))`.
- `schema_version = "v1"`.
- Forward compatibility: a `v2` snapshot MUST be introduced as
  a new type, with explicit migration; v1 stays frozen.
- Resolver / verifier contract:
  - `build_*_snapshot_v1(calculation_output, coefficient_context)`
    returns the typed model;
  - `assert_snapshot_v1(snapshot)` re-parses via the model and
    recomputes the hash; mismatch is `raise SnapshotHashMismatchError`.
...
| Audit outbox (SUCCEEDED / BLOCKED / FAILED) | outer | one terminal event |
| SchemeServiceReadyOutput write | outer | only on SUCCEEDED |

Idempotent replay: if an attempt with the same idempotency key
has already reached a terminal state, the orchestrator MUST
return that state without re-entering the UoW, with
status = `IDEMPOTENT_REPLAY_SUCCEEDED`.
...
| SourceBindingVerifier rejects | FAILED, attempt FAILED, no archive, no scheme ready output, no outbox SUCCEEDED |
| Outbox envelope missing | FAILED, attempt FAILED, no scheme ready output |
...
warn-and-continue. Every fail-closed path is paired with a test
in §11.
...
- `SourceBindingVerifier` rejects missing slot, hash mismatch,
  partial binding.

### 11.2 Integration SQLite

- Orchestrator runs an approved ProjectVersion to SUCCEEDED
  end-to-end; writes 5 calculation runs, 1 binding (verified),
  1 source archive, 1 outbox SUCCEEDED event, 1
  `scheme_service_ready_outputs` row.
- Replay with same idempotency key returns
  IDEMPOTENT_REPLAY_SUCCEEDED, no new rows.
- Demo coefficient path raises
  `CoefficientNotApprovedError`, attempt → BLOCKED, no
  calculation runs, no archive.

### 11.3 Integration PostgreSQL

- Same as SQLite above, on PostgreSQL.
- Concurrent replay with same idempotency key from two
  sessions: exactly one attempt reaches SUCCEEDED; the
  other receives `IDEMPOTENT_REPLAY_SUCCEEDED`.
...
- `SchemeService` (existing) consumes the
  `scheme_service_ready_outputs` row unchanged and
  produces a `SchemeRun` with the same five snapshots
  frozen into its envelope.
...
- The evaluation runner calls the orchestrator (NOT an
  ...
  reaches outcome = `success` without
  `requires_review` warnings reaching the runner.
...
1. **Phase 0 — Schema (separate task)**: Alembic migration
   ...
   `correlation_id`, `actor_principal_type` on
   `orchestration_attempts`, plus a new
   `scheme_service_ready_outputs` table.
...
3. **Phase 2 — SourceBinding & archive**: introduce
   `frozen_envelope` of the binding, wire
   `SourceBindingVerifier` into the orchestrator flow.
4. **Phase 3 — Demo coefficient resolution**: thread
   ...
   are filtered at the adapter boundary.
5. **Phase 4 — SchemeService bridge**: add
   `SqlSchemeServiceReadyOutputAdapter`; expose a public
   API/CLI endpoint.
6. **Phase 5 — Task 11 Phase B unblock**: re-run the
...
This design reuses the following existing modules — see their
source for behavior:
