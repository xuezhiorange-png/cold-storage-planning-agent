...
**状态：** 已发布（V1.8 per-zone temperature and height surface）  
**产品标签：** `v1.8.0` at `ae3814f`（闸门块保持历史冻结）  
...
`V18_IMPLEMENTATION_AUTHORIZED=YES`：本文件冻结「1.8 温度/层高核对什么」。产品标签
`v1.8.0` 已打在 `ae3814f`。本文件闸门保持历史冻结，不改写成 V1.9。
不移动 `v1.7.0` / `v1.8.0`。
...
产品身份仍是 **豆包工作伙伴**。V1.7 已发布。V1.8 让操作者能按区看到冷却真正用的 **室内设计温度 °C** 和 **层高 m**。内核公式不动，`cooling_load@1.0.0` 不 bump。Charles 已批：**九个制冷分区层高一律 4.0 m**；室内设计温度**跟分区规划温区走，区间取低端**（8~10℃ → 8.0；1~3℃ → 1.0；−18℃ → −18.0）。货品目标温度跟室内设计温度同一张表。**已发布 `v1.8.0`。**
...
4. Charles 已回复 **「可以」**；产品标签 `v1.8.0` 已打在 `ae3814f`。不移动 `v1.7.0` / `v1.8.0`。
