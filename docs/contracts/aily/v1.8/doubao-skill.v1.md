...
> V1.7 技能 `docs/contracts/aily/v1.7/**`、V1.6 技能 `docs/contracts/aily/v1.6/**`、V1.5 技能 `docs/contracts/aily/v1.5/**`
...
室内设计温度取分区规划温区低端（8 / 1 / −18 ℃），层高为演示目录 4.0 m，货品目标温度与室内设计温度相同；货品质量仍为 v05 演示目录，需复核。
分区表含室内设计温度与层高列。不要自己推墙面积、屋面面积、ΔT 或分区冷量。
...
3. 必须向用户说明：
   ...
   - 冷量表：**分区冷量按内核五项加总**；**地板、墙、屋面来自分区几何（正方形平面 + 演示层高）**；U 值与设计温度仍为演示目录；室内设计温度取分区规划温区低端（8 / 1 / −18 ℃），层高为演示目录 4.0 m，货品目标温度与室内设计温度相同，货品质量仍为 v05 演示目录，需复核。
