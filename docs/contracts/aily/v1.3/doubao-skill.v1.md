...
### 冷负荷特别说明（地板来自分区；墙屋面仍演示）

冷量预览在内存中把分区 `required_area_m2` 绑定到冷负荷 `floor_area` / `zone_area`（与工作台血缘一致）。
墙、屋面面积与 U 值仍来自**演示目录**，不是几何自动推导。
向用户说明：这是概念设计、需人工复核、演示系数、`requires_review=true`。

成功响应标志：`floor_area_from_zone_plan: true`，`envelope_wall_roof_from_plan: false`。

### 设备与功率特别说明（内存血缘，不用 COP）

- **设备**：预览在内存中把冷负荷各分区 `subtotal_load_kw_r` **字段拷贝**到设备输入（与工作台同源，不落库）。
- **装机功率**：压缩机电气 `compressor_input_power_kw_e` 来自设备计算器输出的 `total_compressor_input_power_kw_e`，**不是** kW(r)/COP 推算。蒸发/冷凝风机电气可能仍为演示目录。
- **投资**：面积与功率来自分区规划合计与装机功率结果，**不是** v05 演示占位。
...
3. 必须向用户说明：
   ...
   - 冷量表：**地板/规划面积来自分区结果**；墙、屋面、U 值仍是演示目录。
   - 功率表：`power_from_demo_catalog: false` 表示压缩机电气来自设备；风机可能仍用演示目录。
