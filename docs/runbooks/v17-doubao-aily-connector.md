# V1.7 豆包工作伙伴五阶段预览 MCP 接入操作手册

本手册供 Charles 在**豆包工作伙伴**里配置 V1.7 五阶段对话预览（分区冷量五项核对）。
V1.6 手册 `docs/runbooks/v16-doubao-aily-connector.md` 与技能 `docs/contracts/aily/v1.6/**`
保持冻结历史；本文件描述 V1.7 增量。飞书请改粘贴 V1.7 技能。

## 范围与边界

- 五阶段预览是**入站传输**，不是第二套内核；`persisted: false`；不是 Transaction B。
- `FORMULA_RECUT_AUTHORIZED=NO`：本版不授权新的公式重切。V1.5 墙/屋面几何绑定保持已发布。
- `ZONE_THERMAL_CATALOG_RECUT=NO`：不把每区 20 t / −18°C 演示热工改成分区真热工。
- **冷量**：分区冷量按内核五项加总（传热 / 产品 / 渗透 / 内部 / 化霜 + 小计），
  与工作台 `COOLING_ZONE_COLUMNS` 同一组字段。成功标志：`floor_area_from_zone_plan: true`，
...
- `docs/contracts/aily/v1.7/doubao-skill.v1.md`（粘贴到豆包技能）
- `docs/contracts/aily/v1.7/doubao-skill.v1.json`（机器可读元数据）

历史冻结：`docs/contracts/aily/v1.6/**`、`docs/runbooks/v16-doubao-aily-connector.md`。
...
以及「分区冷量按内核五项加总」。`extra_tables` 分区表列须含传热 / 产品 / 渗透 / 内部 / 化霜 / 小计。
