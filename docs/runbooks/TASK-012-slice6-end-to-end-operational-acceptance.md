...
### Observability request identity

The S6-07 observability probe reuses the production middleware contract exactly:

- request header: `X-Request-ID`;
- request identity: a valid UUIDv4;
- response header: `X-Request-ID`;
- structured JSON fields: `correlation_id` and `request_id` carry the same UUIDv4.

The workflow does not define a new correlation-header contract. `X-Correlation-ID`
is not the current canonical API header, and a GitHub run ID is not a request or
correlation identity. The expected identity is generated by the probe, while the
response header and structured-log counters are read from the running production
runtime. Missing or mismatched raw observations remain fail-closed verifier inputs.
...
6. Verify the disabled planning-agent boundary, the `X-Request-ID` request ID,
