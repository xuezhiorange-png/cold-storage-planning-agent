# V1.5 豆包工作伙伴五阶段预览 MCP 接入操作手册

本手册供 Charles 在**豆包工作伙伴**里配置 V1.5 五阶段对话预览（墙/屋面从分区几何进入）。
V1.3 手册 `docs/runbooks/v13-doubao-aily-connector.md` 与技能 `docs/contracts/aily/v1.3/**` 保持冻结历史；本文件描述 V1.5 增量。
V1.4 未发新技能；飞书请改粘贴 V1.5 技能。

## 范围与边界

- 五阶段预览是**入站传输**，不是第二套内核；`persisted: false`；不是 Transaction B。
- `FORMULA_RECUT_AUTHORIZED=YES`，且仅限
  `COOLING_LOAD_FORMULA_RECUT=envelope_wall_roof_geometry_only`：
  墙/屋面面积从分区几何进入；**不** bump `cooling_load@1.0.0`。
- **冷量**：内存绑定分区 `required_area_m2` → `floor_area` / `zone_area` /
  `roof_area`，以及正方形平面墙面积（演示层高 5.0 m）。
  成功标志：`floor_area_from_zone_plan: true`，`envelope_wall_roof_from_plan: true`，
  `formula_recut_authorized: true`。U 值与设计温度仍为演示目录。
...
2. `preview_cooling_load` — 冷负荷（地板/墙/屋面来自分区几何；U 值演示目录）
...
`X-Aily-Connector-Key` 与 V1.3 相同。

## 技能包

- `docs/contracts/aily/v1.5/doubao-skill.v1.md`（粘贴到豆包技能）
- `docs/contracts/aily/v1.5/doubao-skill.v1.json`（机器可读元数据）
...
`formula_recut_authorized` 为 true；`markdown_table` 含正方形平面 + 演示层高，以及 U 值演示目录说明。

换一组更大的每天进货量再调一次，冷量应随面积变，不再是全局墙 200 / 屋面 100。
...
- 概念设计、需复核、演示系数、不是施工图。
- 冷量：**地板、墙、屋面来自分区几何（正方形平面 + 演示层高）**；U 值与设计温度仍为演示目录，需复核。
