"""Seed the V0.8 operator-minimal process-input sample through current main public APIs.
...
SAMPLE_ID = "v08-process-input"
MANIFEST_RELATIVE_PATH = Path("samples") / SAMPLE_ID / "manifest.json"
REPO_ROOT = Path(__file__).resolve().parents[4]
TRUSTED_ACTOR = "v08-local-trusted-reviewer"
...
def load_manifest(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"V0.8 process-input sample manifest not found: {path}")
...
def verify_v08_sample(database_url: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="v08-sample-artifacts-") as artifact_tmp:
        ...
        with trusted_sample_client(database_url, storage_dir=storage_dir) as (reopened, _service):
            ...
            reseeded = seed_v08_sample(reopened)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Seed or verify the V0.8 operator-minimal process-input sample via current main APIs. "
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Run full operator process-input checks (review, formal exports, restart).",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify:
        ...
        if args.json:
            ...
            print("V08_SAMPLE_VERIFY=ok")
            print(f"V08_SAMPLE_PROJECT_ID={summary['project_id']}")
            print(f"V08_SAMPLE_REPORT_ID={summary['report_id']}")
            print(f"V08_SAMPLE_SCHEME_RUN_ID={summary['production_scheme_run_id']}")
        return 0
    ...
    if args.json:
        ...
        print(f"V08_SAMPLE_PROJECT_ID={seeded.project_id}")
        print(f"V08_SAMPLE_PROJECT_CODE={seeded.project_code}")
        print(f"V08_SAMPLE_VERSION={seeded.version_number}")
        print(f"V08_SAMPLE_VERSION_ID={seeded.project_version_id}")
        print(f"V08_SAMPLE_CREATED={'yes' if seeded.created else 'no'}")
        print("V08_SAMPLE_PERSISTED=" + ",".join(seeded.persisted_calculator_names))
        print(
            f"V08_SAMPLE_PRODUCTION_SCHEME_SUCCESS="
            f"{'yes' if seeded.production_scheme.success else 'no'}"
        )
        if seeded.production_scheme.run_id:
            print(f"V08_SAMPLE_SCHEME_RUN_ID={seeded.production_scheme.run_id}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"V08_SAMPLE_ERROR={exc}", file=sys.stderr)
        raise
