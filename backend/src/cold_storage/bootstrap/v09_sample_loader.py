"""Seed the V0.9 operator-minimal process-input sample through current main public APIs.

The loader uses project, five-stage-execution (operator_process_input path),
production-scheme-runs, and report lifecycle endpoints only. It does not invoke
planning-run, legacy scheme-runs, demo scheme-comparison, or Alembic.
...
from cold_storage.bootstrap.v05_local_sample import EXPECTED_CANONICAL_CALCULATORS
...
SAMPLE_ID = "v09-process-input"
MANIFEST_RELATIVE_PATH = Path("samples") / SAMPLE_ID / "manifest.json"
REPO_ROOT = Path(__file__).resolve().parents[4]
TRUSTED_ACTOR = "v09-local-trusted-reviewer"
UNTRUSTED_ACTOR = "system"
TEMPLATE_VERSION = "1.0.0"
RENDER_LOCALES = ("zh-CN", "en-US")
RENDER_FORMATS = ("docx", "pdf")
PRODUCTION_PROFILE_CODES = ("balanced",)

V09_OPERATOR_KEY_LEAVES = frozenset(
    {
...
V09_FORBIDDEN_MANIFEST_KEYS = frozenset(
    {
...
class ReportLifecycleResult:
    ...
    draft_exports: tuple[RenderExportResult, ...] = field(default_factory=tuple)
    formal_exports: tuple[RenderExportResult, ...] = field(default_factory=tuple)
    fail_closed: bool = False
    fail_closed_reason: str | None = None


@dataclass(frozen=True, slots=True)
class SeededV09Sample:
    sample_id: str
    project_id: str
    project_code: str
    version_number: int
...
def load_manifest(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"V0.9 process-input sample manifest not found: {path}")
    ...
    if "engineering_input_bundle" in manifest:
        raise ValueError(
            "manifest must not contain engineering_input_bundle (operator-minimal only)"
        )
    operator_input = manifest.get("operator_process_input")
    if not isinstance(operator_input, dict):
        raise ValueError("manifest.operator_process_input is required")
    if operator_input.get("schema_version") != "1.1.0":
        raise ValueError("manifest.operator_process_input.schema_version must be 1.1.0")
    zone_inputs = operator_input.get("zone_planning_inputs")
    if not isinstance(zone_inputs, dict):
        raise ValueError("manifest.operator_process_input.zone_planning_inputs is required")
    if set(zone_inputs) != V09_OPERATOR_KEY_LEAVES:
        raise ValueError(
            f"manifest must contain exactly V0.9 five KEY leaves; got {sorted(zone_inputs)}"
        )
    for forbidden in V09_FORBIDDEN_MANIFEST_KEYS:
        if forbidden in zone_inputs:
            raise ValueError(f"manifest must not contain forbidden KEY leaf: {forbidden}")
...
def _zone_by_code(zones: list[dict[str, Any]], zone_code: str) -> dict[str, Any]:
    for zone in zones:
        if zone.get("zone_code") == zone_code:
            return zone
    raise RuntimeError(f"zone {zone_code!r} not found in persisted zone plan")


def _zones_from_result_snapshot(result_snapshot: dict[str, Any]) -> list[dict[str, Any]]:
    zones = result_snapshot.get("zones")
    if isinstance(zones, list):
        return cast(list[dict[str, Any]], zones)
    result = result_snapshot.get("result")
    if isinstance(result, dict):
        nested_zones = result.get("zones")
        if isinstance(nested_zones, list):
            return cast(list[dict[str, Any]], nested_zones)
    raise RuntimeError("cold_room_zone_plan result_snapshot must expose zones")


def _assert_v09_zone_snapshot(by_name: dict[str, dict[str, Any]]) -> None:
    zone_row = by_name.get("cold_room_zone_plan")
    if zone_row is None:
        raise RuntimeError("cold_room_zone_plan is missing from persisted calculations")
    result_snapshot = zone_row.get("result_snapshot")
    if not isinstance(result_snapshot, dict):
        raise RuntimeError("cold_room_zone_plan.result_snapshot must be a dict")
    zones = _zones_from_result_snapshot(result_snapshot)
    zone_codes = {zone.get("zone_code") for zone in zones if isinstance(zone, dict)}
...
def _run_report_lifecycle(
    ...
    *,
    ...
    draft_exports_list: list[RenderExportResult] = []
    for locale in RENDER_LOCALES:
        for export_format in RENDER_FORMATS:
            ...
            if rendered.status_code != 200:
                raise RuntimeError(
                    f"draft export failed for {locale}/{export_format}: "
                    f"{rendered.status_code} {rendered.detail}"
                )
            draft_exports_list.append(rendered)
    ...
    if submit.status_code != 200:
        return ReportLifecycleResult(
            ...
            draft_exports=tuple(draft_exports_list),
            fail_closed=True,
    ...
    formal_exports_list: list[RenderExportResult] = []
    for locale in RENDER_LOCALES:
        for export_format in RENDER_FORMATS:
            rendered = _render_report(
                client,
...
def seed_v09_sample(
    ...
    *,
    ...
    operator_process_input = copy.deepcopy(manifest["operator_process_input"])
    ...
    version = _version_record(client, project_id, version_number)
    ...
    if persisted_before >= set(EXPECTED_CANONICAL_CALCULATORS):
        ...
        execution_response = client.post(
            ...
            json={
                "operator_process_input": operator_process_input,
    ...
    _assert_v09_zone_snapshot(by_name)
...
def verify_v09_sample(database_url: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="v09-sample-artifacts-") as artifact_tmp:
        ...
        with trusted_sample_client(database_url, storage_dir=storage_dir) as (client, service):
            ...
            if lifecycle.fail_closed:
                raise RuntimeError(
                    "operator process-input verify expected submit-review 200; got "
            ...
            for export in (*lifecycle.draft_exports, *lifecycle.formal_exports):
                if not export.artifact_id:
                    raise RuntimeError(f"{export.mode} export missing artifact_id")
                detail = client.get(
                ...
                if len(download.content) < 100:
                    raise RuntimeError(
                        f"artifact download too small for "
                        f"{export.mode}/{export.locale}/{export.export_format}"
                    )
        ...
        with trusted_sample_client(database_url, storage_dir=storage_dir) as (reopened, _service):
            ...
            reseeded = seed_v09_sample(reopened)
    ...
    return {
        ...
        "draft_exports": [
            ...
            for item in lifecycle.draft_exports
        ],
        "formal_exports": [
            {
                "locale": item.locale,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Seed or verify the V0.9 operator-minimal process-input sample via current main APIs. "
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Run full operator process-input checks (draft/formal exports, restart).",
    )
    parser.add_argument(
        "--json",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify:
        ...
        if args.json:
            ...
            print("V09_SAMPLE_VERIFY=ok")
            print(f"V09_SAMPLE_PROJECT_ID={summary['project_id']}")
            print(f"V09_SAMPLE_REPORT_ID={summary['report_id']}")
            print(f"V09_SAMPLE_SCHEME_RUN_ID={summary['production_scheme_run_id']}")
        return 0
    ...
    if args.json:
        ...
        print(f"V09_SAMPLE_PROJECT_ID={seeded.project_id}")
        print(f"V09_SAMPLE_PROJECT_CODE={seeded.project_code}")
        print(f"V09_SAMPLE_VERSION={seeded.version_number}")
        print(f"V09_SAMPLE_VERSION_ID={seeded.project_version_id}")
        print(f"V09_SAMPLE_CREATED={'yes' if seeded.created else 'no'}")
        print("V09_SAMPLE_PERSISTED=" + ",".join(seeded.persisted_calculator_names))
        print(
            f"V09_SAMPLE_PRODUCTION_SCHEME_SUCCESS="
            f"{'yes' if seeded.production_scheme.success else 'no'}"
        )
        if seeded.production_scheme.run_id:
            print(f"V09_SAMPLE_SCHEME_RUN_ID={seeded.production_scheme.run_id}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"V09_SAMPLE_ERROR={exc}", file=sys.stderr)
        raise
