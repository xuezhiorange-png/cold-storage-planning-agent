...
from cold_storage.modules.reports.localization.errors import MissingTranslationError
...
def test_chinese_column_key_without_catalog_entry_still_fails_closed() -> None:
    """Regression guard: raw Chinese column keys must not bypass catalog lookup."""
    canonical = build_canonical_render_model(
        content={
            "investment_estimate": {
                "total_investment": 1_000_000.0,
                "breakdown": {"土建及钢结构": 1_000_000.0},
            }
        },
        report_id="investment-regression-test",
        revision_number=1,
        content_hash="hash-regression",
        generated_by="test",
        generated_at="2026-08-27T00:00:00Z",
        template_code="cold_storage_concept_design",
        template_version="1.0.0",
    ...
    assert exc_info.value.key == "investment.土建及钢结构"
