...
class TestTablesExist:
    def test_all_orchestration_tables(self, migrated_engine) -> None:
        ...
        required = {
            "orchestration_requests", "orchestration_execution_snapshots",
            "orchestration_coefficient_contexts", "orchestration_identities",
            "orchestration_run_attempts", "orchestration_source_bindings",
            "orchestration_audit_outbox", "scheme_weight_set_revisions",
        }
...
class TestCheckConstraints:
    def test_calculation_run_check(self, migrated_engine) -> None:
        with migrated_engine.connect() as conn:
            ddl = conn.execute(text(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='calculation_runs'"
            )).scalar()
    ...
    def test_scheme_run_check(self, migrated_engine) -> None:
        with migrated_engine.connect() as conn:
            ddl = conn.execute(text(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='scheme_runs'"
            )).scalar()
    ...
    def test_request_check(self, migrated_engine) -> None:
        with migrated_engine.connect() as conn:
            ddl = conn.execute(text(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='orchestration_requests'"
            )).scalar()
...
class TestIndexes:
    def test_one_running(self, migrated_engine) -> None:
        with migrated_engine.connect() as conn:
            idxs = conn.execute(text(
                "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='orchestration_run_attempts'"
            )).fetchall()
    ...
    def test_source_binding_slots(self, migrated_engine) -> None:
        with migrated_engine.connect() as conn:
            idxs = conn.execute(text(
                "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='orchestration_source_bindings'"
            )).fetchall()
            names = {r[0] for r in idxs}
