"""V0.9 P7 controlled acceptance integration tests (PostgreSQL)."""
...
from cold_storage.bootstrap.v09_sample_loader import trusted_sample_client
from cold_storage.modules.projects.infrastructure.database import create_database_project_service
from tests.integration.test_v09_p7_controlled_acceptance_sqlite import (
...
def test_v09_p7_pg_controlled_acceptance(pg_database: str, tmp_path: Path) -> None:
...
def test_v09_p7_pg_missing_operator_key_fail_closed(
    pg_database: str,
    tmp_path: Path,
    leaf_name: str,
    _label: str,
    ...
    with isolated_process_state():
        ...
        with TestClient(create_app(project_service=service)) as client:
            ...
            created = client.post(
                "/api/v1/projects",
                json={
                    "name": f"V09-P7 pg missing key {uuid.uuid4().hex[:8]}",
                    "location": "山东",
...
@pytest.mark.postgresql
def test_v09_p7_pg_demo_catalog_leaves_remain_unverified(
    pg_database: str,
    tmp_path: Path,
) -> None:
