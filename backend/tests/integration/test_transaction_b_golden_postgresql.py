"""Cross-backend golden parity test — PostgreSQL.

Uses real PostgreSQL with Alembic Head (via conftest.py fixtures).
Seeds golden prerequisites directly, runs Transaction B with
FixedTransactionBIdFactory, reads back from DB, and compares with
the same golden JSON artifact as the SQLite test.

Must produce IDENTICAL artifact to SQLite (since all IDs are fixed).
...
if os.environ.get("DATABASE_BACKEND") != "postgresql":
    pytest.skip(
        "PostgreSQL golden parity tests require DATABASE_BACKEND=postgresql",
...
@pytest.fixture()
def service(pg_session_factory):
...
class TestGoldenParityPostgreSQL:
    """Golden parity tests that run on PostgreSQL."""
    ...
    def test_golden_transaction_b_produces_canonical_artifact(
        self, service, pg_session_factory
    ) -> None:
