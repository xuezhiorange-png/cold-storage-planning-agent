"""V0.9 P6 operator-minimal sample integration tests (PostgreSQL)."""
...
from cold_storage.bootstrap.v09_sample_loader import trusted_sample_client, verify_v09_sample
...
if os.environ.get("DATABASE_BACKEND") != "postgresql":
    pytest.skip(
        "P6 PostgreSQL integration tests require DATABASE_BACKEND=postgresql",
        allow_module_level=True,
    )


@pytest.mark.postgresql
def test_v09_p6_unmodified_create_app_five_stage_postgresql(
    pg_database: str,
    tmp_path: Path,
) -> None:
...
@pytest.mark.postgresql
def test_v09_p6_postgresql_env_and_singletons_isolated_before_create_app(
    pg_database: str,
    tmp_path: Path,
) -> None:
...
@pytest.mark.postgresql
def test_v09_p6_verify_loader_postgresql(pg_database: str, tmp_path: Path) -> None:
    artifact_dir = tmp_path / "pg-verify-artifacts"
    artifact_dir.mkdir()
    with isolated_process_state():
        configure_postgresql_env(pg_database, artifact_dir)
        summary = verify_v09_sample(pg_database)
