...
# ── Commit D negative-policy tests (TASK-011B §7) ────────────────────────


def test_negative_policy_30_exact_proxy_overlap_fails() -> None:
    """Neg test 30: adding ``$.source_binding_proxy`` (a normalized
    proxy canonical path) to BOTH ``exact_match_fields`` and
    ``normalized_proxy_fields`` MUST be rejected with
    ``POLICY_EXACT_PROXY_OVERLAP``."""
    from tests.evaluation._seed_helpers import validate_expected_output_comparison_policy
    ...
    # proxy already has it (it's the canonical normalized proxy path)
    try:
        validate_expected_output_comparison_policy(g, backend="sqlite")
    except AssertionError as exc:
        msg = str(exc)
...
def test_negative_policy_31_proxy_excluded_overlap_fails() -> None:
    """Neg test 31: adding a normalized proxy path to BOTH
    ``normalized_proxy_fields`` and ``excluded_runtime_fields`` MUST be
    rejected with ``POLICY_PROXY_EXCLUDED_OVERLAP``."""
    from tests.evaluation._seed_helpers import validate_expected_output_comparison_policy
...
def test_negative_policy_32_dup_proxy_fails() -> None:
    """Neg test 32: duplicating an entry in ``normalized_proxy_fields``
    MUST be rejected with ``POLICY_DUP_PROXY``."""
    from tests.evaluation._seed_helpers import validate_expected_output_comparison_policy
...
def test_negative_policy_33_scenario_outcome_derived_from_runtime() -> None:
    """Neg test 33: ``build_baseline_expected_output_actual`` MUST
    derive ``expected_outcome`` from the live ``ScenarioOutcome.outcome``,
    NOT hard-code ``"SUCCEEDED"``.

    We pass a stub ``ScenarioOutcome(outcome="REVIEW_REQUIRED")`` and
    a stub ``SchemeRunRecord`` carrying the relevant fields. The
    resulting canonical actual must report ``expected_outcome ==
    "REVIEW_REQUIRED"``.
    """
    from tests.evaluation._seed_helpers import (
...
def test_negative_policy_34_warning_messages_not_excluded_but_mapped() -> None:
    """Neg test 34: ``scheme_run.warning_messages`` MUST NOT appear in
    ``excluded_runtime_fields`` (it is mapped to ``review_reasons``),
    but the ``field_normalization_mapping`` MUST document the
    mapping."""
    g = _negative_golden()
    p = g["_comparison_policy"]
